...
| `debug-triage` | Systematically narrowing a non-obvious bug: reproduce, isolate, hypothesize, test, verify. | An obvious single-cause bug — just fix it directly; reviewing working code (`code-review`); explaining code behavior. |
| `document-formatter` | Restructuring rough, disorganized, multi-topic notes into a properly organized document with real section headings. | A small bullets-to-one-paragraph turn (`writing-assistant`); condensing an already-polished source (`source-summarizer`); producing a specific file format like `.docx` (the `docx` skill). |
| `research-brief` | Sourced research briefs on open-ended questions/comparisons: findings labeled by confidence, consensus vs. disagreement shown explicitly, gaps stated. | A settled/immediate-answer question; summarizing one given source (`source-summarizer`); a request for opinion rather than sourced findings. |
| `fount-gamedev` | Authoring game content for the bundled Fount engine (`apps/fount`): levels, entity placement, IO logic wiring, custom entity behaviours, level debugging. | Game development in any other engine (Unity, Godot, Unreal, three.js); general game-design discussion; editing the Fount engine's own internals under `apps/fount/engine/`, which is ordinary repo code work. |

Deliberate non-duplications, since this environment already ships these: `code-review`, `init`, and `security-review` are Anthropic-provided skills already available — nothing in this catalog reimplements them. `repo-onboarding` sits next to `init` but does a different job (explaining the codebase to a human conversationally vs. writing a persistent instructions file); `debug-triage` sits next to `code-review` but investigates a live failure rather than reviewing working code; `email-drafter` sits next to `writing-assistant` but only handles the subset of email requests where angle is a genuine open question; `document-formatter` sits next to both `writing-assistant` (smaller/shorter pieces) and `docx` (this skill structures content, not a file format).

A `commit-and-pr` skill was considered for this batch and deliberately deferred: Claude Code's own default behavior already drafts commit messages and PR descriptions by reading repo conventions, and a thin wrapper around that would be exactly the "wrapper skill" anti-pattern in `docs/PLAN.md` Section 17. It's worth revisiting only if there's a concrete case for applying a *named* convention (e.g. Conventional Commits) that the default repo-style-matching behavior doesn't cover — not before.
