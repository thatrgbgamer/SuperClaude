...
description: Drafts or edits general prose — routine emails, short posts, bios, announcements, farewell messages, cover letter paragraphs, or any other written text — matching the user's voice, formatting, and formality preferences instead of generic AI phrasing. Use when the user asks to draft, write, rewrite, edit, polish, or punch up a piece of text that isn't code, a commit message, or a summary of existing source material. Also use for turning rough notes or bullet points into prose. Do not use for writing code, summarizing an existing article/transcript/document, drafting commit messages or PR descriptions, or reviewing code — those are separate, more specific skills. Also do not use for cold outreach, follow-ups after silence, declines, negotiation asks, or any email where the strategic angle itself is the open question — that's email-drafter, which produces multiple angled variants instead of one draft.
version: 0.1.0
---
